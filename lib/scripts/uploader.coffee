...
#jQuery(document).on 'ready page:load', ->
  #if jQuery('.upload_info a.upload').length > 0
    #ele = jQuery('.upload_info a.upload')
    #files = jQuery('.upload_info .files')
    ##drag_area = jQuery('.page-file-entities .drag-area')
    #new Uploader(ele, files)

class ExtUploader
  constructor: (@$upload_button, @$uptoken)->
    data = @$upload_button.data()
    @domain = data['domain']
    @basepath = data['basepath']
    @_init()
...
class UptokenManager
  constructor: (@$func)->
    @get(@$func)

  get: (func) ->
    console.log 'start get uptoken'
    $.ajax 'http://done-list.local.realityandapp.com:3000/file_entities/uptoken'
      .done (res) =>
        console.log 'got uptoken'
        console.log(res)
      .fail (err) =>
        console.log err
      .always =>
